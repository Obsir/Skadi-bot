...
from skadi.utils.bot_database import DBUser, DBGroup, DBAuth
from skadi.utils.plugin_utils import init_export


# Custom plugin usage text
__plugin_name__ = 'SkadiAuth'
__plugin_usage__ = r'''【SkadiAuth 授权管理插件】
插件特殊权限授权管理
仅限管理员使用
...
# 修改默认参数处理
@skadiauth.args_parser
async def parse(bot: Bot, event: MessageEvent, state: T_State):
    args = str(event.get_plaintext()).strip().split()
    if not args:
        await skadiauth.reject('你似乎没有发送有效的参数呢QAQ, 请重新发送:')
    state[state["_current_key"]] = args[0]
    if state[state["_current_key"]] == '取消':
        await skadiauth.finish('操作已取消')
...
async def handle_first_receive(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if not args:
        ...
        await skadiauth.finish('参数错误QAQ')


@skadiauth.got('sub_command', prompt='执行操作?\n【allow/deny/clear/list】')
...
# 处理显示权限节点列表事件
@skadiauth.got('sub_command', prompt='list:')
async def handle_list_node(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if sub_command == 'list':
        ...
        if detail_type == 'group':
            ...
            await skadiauth.finish('非授权会话, 操作中止')


@skadiauth.got('auth_type', prompt='授权类型?\n【user/group】')
...
async def handle_auth_id(bot: Bot, event: MessageEvent, state: T_State):
    ...
    await skadiauth.send(f'可配置权限节点的插件有:\n\n{p_list}')


@skadiauth.got('plugin', prompt='请输入想要配置权限节点的插件名称:')
async def handle_plugin(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if plugin not in auth_node_plugin:
        await skadiauth.reject('插件名称错误, 请重新输入:')

    plugin_auth_nodes = get_plugin(plugin).export.get('auth_node')
    state["plugin_auth_nodes"] = plugin_auth_nodes
    an_list = '\n'.join(plugin_auth_nodes)
    await skadiauth.send(f'{plugin}可配置的权限节点有:\n\n{an_list}')


@skadiauth.got('auth_node', prompt='请输入想要配置的权限节点名称:')
async def handle_auth_node(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if auth_node not in plugin_auth_nodes:
        await skadiauth.reject('权限节点名称错误, 请重新输入:')
    ...
    if res.success():
        ...
        await skadiauth.finish(f'{auth_type}/{auth_id} {r_auth_node} 权限节点 {sub_command} 操作成功')
    else:
        logger.error(f'为 {auth_type}/{auth_id} {sub_command} 权限节点 {r_auth_node} 失败: {res.info}')
        await skadiauth.finish(f'{auth_type}/{auth_id} {r_auth_node} 权限节点 {sub_command} 操作失败QAQ, 请稍后再试')
