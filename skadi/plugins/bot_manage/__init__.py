...
- /Skadi init - bot首次加入新群组/首次添加bot好友后, 须使用本命令进行初始化
- /Skadi upgrade - 手动更新本群组信息
- /Skadi notice <on|off> - 为本群组配置通知权限(订阅类插件是否通知)
- /Skadi command <on|off> - 为本群组配置命令权限(是否允许使用命令)
- /Skadi setlevel <PermissionLevel> - 为本群组配置命令等级(对于低于命令要求等级的群组, 该命令不会被响应)
"""
...
from skadi.utils.bot_database import DBGroup, DBUser, DBAuth, DBFriend, Result
from skadi.utils.plugin_utils import init_export
from .sys_background_scheduled import scheduler

# Custom plugin usage text
__plugin_name__ = 'Skadi'
__plugin_usage__ = r'''【Skadi 管理插件】
Skadi机器人管理
...
/Skadi upgrade
/Skadi notice <on|off>
/Skadi command <on|off>
/Skadi setlevel <PermissionLevel>
/Skadi showpermission
/Skadi resetpermission
...
# 注册事件响应器
skadi = on_command('Skadi', rule=None, aliases={'skadi'},
...
async def parse(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if not args:
        await skadi.reject('你似乎没有发送有效的参数呢QAQ, 请重新发送:')
    state[state["_current_key"]] = args[0]
    if state[state["_current_key"]] == '取消':
        await skadi.finish('操作已取消')
...
async def handle_first_receive(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if args and len(args) == 1:
        ...
        await skadi.finish('你好呀~ 我是Skadi~ 请问您今天要来点喵娘吗?')


@skadi.got('sub_command', prompt='执行操作?\n【init/upgrade/notice/command/setlevel/showpermission/resetpermission】')
async def handle_sub_command(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if result.success():
        ...
        if sub_command in need_reply:
            await skadi.finish(result.result)
        else:
            await skadi.finish('Success')
    else:
        logger.error(f"Group: {event.group_id}, {sub_command}, Failed, {result.info}")
        await skadi.finish('Failed QAQ')
...
async def friend_private_enable(bot: Bot, event: PrivateMessageEvent, state: T_State) -> Result.TextResult:
    ...
    if result.success():
        ...
        return Result.TextResult(error=True, info=result.info, result='失败, 请先尝试"/Skadi init", 若仍失败请联系管理员处理')
...
async def init_group_auth_node(group_id: int):
    ...
    default_auth_nodes = [
        AuthNode(node='help.skip_cd', allow_tag=1, deny_tag=0, auth_info='默认规则: help免cd'),
...
async def init_user_auth_node(user_id: int):
    ...
    default_auth_nodes = [
        AuthNode(node='help.skip_cd', allow_tag=1, deny_tag=0, auth_info='默认规则: 好友help免cd')
    ]
