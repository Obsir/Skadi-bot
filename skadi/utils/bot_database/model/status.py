            ...
            async with session.begin():
                try:
                    session_result = await session.execute(
                        select(Status.status).where(Status.name == self.name)
                    )
            ...
            try:
                async with session.begin():
                    try:
                        # 已存在则更新
                        session_result = await session.execute(
                            select(Status).where(Status.name == self.name)
                        )
                        ...
                        # 不存在则添加信息
                        new_status = Status(name=self.name, status=status, info=info, created_at=datetime.now())
